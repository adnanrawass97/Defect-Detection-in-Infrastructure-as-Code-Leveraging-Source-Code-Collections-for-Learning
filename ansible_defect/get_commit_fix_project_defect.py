...
CHANGES_FILE = 'szz-validation-commit-changes.csv'

def fetch_commit_details(owner, repo_name, ref,file_path):
        ...
        for file_info in files:
            ...
            if filename.lstrip() == file_path.lstrip():
                if patch is not None:  # Check if patch exists
                    ...
                    yield (ref, repo_name, filename, removed_lines_text, added_lines_text)
...
def process_commit(repo, commit_id, file_path, decision):
    if '/' not in repo:
        print(f"Error: Invalid repository format '{repo}'. It should be in the format 'owner/repo'.")
        return

    owner, repo_name = repo.split('/')
    commit_details = fetch_commit_details(owner, repo_name, commit_id,file_path)
...
def main():
    with open(CHANGES_FILE, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Commit_id", "Repo_name", "File_path", "Deleted_lines_fixing_commit", "Added_lines_fixing_commit"])
    ...
    with open(CSV_FILE, 'r') as csvfile:
        ...
        for row in reader:
            ...
            time.sleep(0.2)
            process_commit(repo, fixing_commit, file_path, decision)
